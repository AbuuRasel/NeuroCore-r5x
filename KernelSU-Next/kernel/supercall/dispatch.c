...
static int do_grant_root(void __user *arg)
{
    ...
    /* B6m: restore real grant (B6j state) */
...
static int do_get_wrapper_fd(void __user *arg)
{
    ...
    /* BISECT B6a: file_wrapper bypass */
    pr_info("B6a: wrapper fd skipped\n");
    return -EINVAL;
#endif
    if (!ksu_file_sid) {
        return -EINVAL;
...
#ifdef CONFIG_KSU_MANUAL_HOOK
    /* B6r: + SET_SEPOLICY (custom contexts, module/base rules). Additive-only
     * livepatch on real 4.14 selinux_state API. */
    if (cmd != KSU_IOCTL_GET_INFO && cmd != KSU_IOCTL_GET_INFO_LEGACY &&
        cmd != KSU_IOCTL_GET_HOOK_MODE && cmd != KSU_IOCTL_GET_VERSION_TAG &&
        cmd != KSU_IOCTL_GET_FEATURE && cmd != KSU_IOCTL_SET_FEATURE &&
        cmd != KSU_IOCTL_CHECK_SAFEMODE && cmd != KSU_IOCTL_SET_SEPOLICY &&
        cmd != KSU_IOCTL_GRANT_ROOT &&
        cmd != KSU_IOCTL_GET_ALLOW_LIST && cmd != KSU_IOCTL_NEW_GET_ALLOW_LIST &&
        cmd != KSU_IOCTL_GET_DENY_LIST && cmd != KSU_IOCTL_NEW_GET_DENY_LIST &&
        cmd != KSU_IOCTL_UID_GRANTED_ROOT && cmd != KSU_IOCTL_UID_SHOULD_UMOUNT &&
        cmd != KSU_IOCTL_GET_MANAGER_APPID &&
