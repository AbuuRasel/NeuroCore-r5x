...
static bool is_init_rc(struct file *fp)
{
    ...
    // NeuroCore: also match ramdisk /init.rc. Some A10 (non-hw-init.rc
    // layout) ROMs have no /system/etc/init/hw/init.rc at all and init
    // reads the ramdisk /init.rc instead; without this the rc hook never
    // fires and no post-fs-data/services stage ever runs.
...
#ifdef CONFIG_KSU_MANUAL_HOOK
    pr_info("ksud: manual hook mode, skipping table hooks and input kprobe\n");
#else
