...
/* 4.14 compat chain for legacy register_random_ready_notifier()
 * consumers (defined below). */
static BLOCKING_NOTIFIER_HEAD(lrng_ready_chain);

/* 4.14 backport: struct random_ready_callback lives in
 * include/linux/random.h since 5.x; define locally (same layout). */
...
void lrng_process_ready_list(void)
{
	...
	/* 4.14 compat: wake legacy notifier consumers (vsprintf, random32)
	 * waiting for a seeded RNG. */
	blocking_notifier_call_chain(&lrng_ready_chain, 0, NULL);
}
...
size_t __must_check get_random_bytes_arch(void *buf, size_t nbytes)
{
	...
	while (left) {
		unsigned long v;
		int chunk = min_t(size_t, left, sizeof(unsigned long));
	...
	if (left)
		lrng_sdrng_get_atomic(p, (u32)left);

	return nbytes - left;
}
EXPORT_SYMBOL(get_random_bytes_arch);

/***************** 4.14 compat: legacy random.c interfaces *****************
 * With CONFIG_LRNG=y, drivers/char/random.o is not built, so LRNG must
 * provide the legacy symbols the 4.14 core expects. */
...
int __init random_init(const char *command_line)
{
	...
	/* Early seeding, mirroring legacy random.c behavior. */
	for (i = 0; i < 4; i++) {
		if (!arch_get_random_long_early(&entropy) &&
		    !arch_get_random_long(&entropy))
			entropy = random_get_entropy();
		lrng_pool_lfsr_nonaligned((u8 *)&entropy, sizeof(entropy));
	}
	if (command_line && command_line[0])
		lrng_pool_lfsr_nonaligned((u8 *)command_line,
					  (u32)strlen(command_line));
	return 0;
}

bool rng_is_initialized(void)
{
	return lrng_state_fully_seeded();
}
EXPORT_SYMBOL(rng_is_initialized);

/* Legacy ready-notifier (lib/vsprintf.c, lib/random32.c): 4.14 core has
 * no random_ready_callback struct, so serve them with the classic
 * blocking notifier chain (head defined above) fired from
 * lrng_process_ready_list(). */

int register_random_ready_notifier(struct notifier_block *nb)
{
	/* Match legacy semantics: -EALREADY makes callers run inline. */
	if (lrng_state_fully_seeded())
		return -EALREADY;
	return blocking_notifier_chain_register(&lrng_ready_chain, nb);
}
EXPORT_SYMBOL(register_random_ready_notifier);

int unregister_random_ready_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&lrng_ready_chain, nb);
}
