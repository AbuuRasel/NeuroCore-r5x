...
/* NeuroCore 4.14: upstream v3.4.0 made this static, but setuid_hook.c
 * still calls it cross-TU on old kernels (upstream CI only covers
 * 5.10+). Keep global linkage like pre-v3.4.0. */
void disable_seccomp(void)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 9, 0)
    /* 4.14: legacy disable path */
...
int escape_with_root_profile(void)
{
    ...
    /* BISECT B6l: full body + COMMIT (real root), caller returns error so the
     * manager stays on the unrooted path (no post-grant root flows).
     * Crash   => commit_creds itself (or uid-0 normal queries).
     * Survive => grant-SUCCESS-only flows (su succeeding etc.). */
    commit_creds(cred);
    ...
    /* BISECT B6h: skip tracepoint-marking loop entirely (was RCU-fixed in
     * B6f, still crashed). Tests whether TIF_SYSCALL_TRACEPOINT marking of
     * all manager threads from ioctl context is the crash vector. */
