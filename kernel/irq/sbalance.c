...
/*
 * SBalance - simple IRQ balancer, 4.14 backport for NeuroCore trinket.
 *
 * Original: Copyright (C) 2023-2024 Sultan Alsawaf <sultan@kerneltoast.com>.
 * 4.14 adaptations (NeuroCore sm8150sync):
 *  - 4.14 struct irq_desc has no ->last_cpu; current CPU is derived from
 *    desc->irq_common_data.affinity instead.
 *  - 4.14 __irq_can_set_affinity() is static in manage.c; equivalent logic
 *    is open-coded here with public accessors.
 *  - irq_set_affinity_locked() is global in manage.c but not declared in
 *    4.14 internals.h, so it is declared extern here.
 */
...
/* Defined in kernel/irq/manage.c (not declared in 4.14 internals.h) */
...
void sbalance_desc_add(struct irq_desc *desc)
{
	...
	*bi = (typeof(*bi)){ .desc = desc, .prev_cpu = -1 };
...
/* 4.14 equivalent of __irq_can_set_affinity() (static in manage.c) */
static bool sbalance_irq_movable(struct irq_desc *desc)
{
...
static bool update_irq_data(struct bal_irq *bi, int *cpu)
{
	...
	/*
	 * 4.14 has no desc->last_cpu; use the first CPU of the current
	 * affinity mask as the presumed home of this IRQ.
	 */
...
static void balance_irqs(void)
{
	...
	for_each_cpu(cpu, &cpus) {
		...
		if (unlikely(!per_cpu(cpu_cap, cpu)))
			per_cpu(cpu_cap, cpu) = SCHED_CAPACITY_SCALE;
	...
	list_for_each_entry_rcu(bi, &bal_irq_list, node) {
		...
		if (!sbalance_irq_movable(bi->desc))
			continue;
