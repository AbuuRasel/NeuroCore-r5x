...
#else
/* 4.14: direct syscall, int args need no user pointers */
static long ksu_sys_setns(int fd, int flags)
{
...
#else
    /* 4.14: no path_mount, remount via path string */
    char *rbuf = kmalloc(PATH_MAX, GFP_KERNEL);
    int pm_ret = -ENOMEM;
    if (rbuf) {
        char *rpath = d_path(&root_path, rbuf, PATH_MAX);
        if (!IS_ERR(rpath)) {
            mm_segment_t old_fs = get_fs();
            set_fs(KERNEL_DS);
            pm_ret = (int)sys_mount(NULL, (char __user *)rpath, NULL,
                                    MS_PRIVATE | MS_REC, NULL);
            set_fs(old_fs);
        } else {
            pm_ret = PTR_ERR(rpath);
        }
        kfree(rbuf);
    }
#endif
...
{
    /* NeuroCore: B3 bypass removed. INHERITED (default profile) is a no-op,
     * so behavior is unchanged unless a per-app ns mode is configured. */
    // inherit mode
