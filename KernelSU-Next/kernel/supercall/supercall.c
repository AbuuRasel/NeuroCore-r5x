...
extern int ksu_handle_susfs_reboot(unsigned int cmd, void __user **arg);
#endif // utsname() and uts_sem
...
#ifdef CONFIG_KSU_SUSFS
    /* FIX: pass kernel address of arg (proper double-pointer). Direct cast of
     * the user value made funcs deref struct bytes; &arg lives in kernel
     * memory so *user_info is PAN-safe and yields the real user pointer. */
    if (magic2 == SUSFS_MAGIC) {
        if (current_uid().val != 0)
            return 0;
        return ksu_handle_susfs_reboot(cmd, (void __user **)&arg);
    }
#endif
#else
...
#else
    pr_info("manual hook mode: reboot kprobe skipped\n");
#endif
}
