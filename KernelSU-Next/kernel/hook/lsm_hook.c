...
#else
/* 4.14 uses struct list_head for LSM hook lists; the hlist-based live
 * patching framework is not applicable. Its only consumer (selinux_hide)
 * is 5.x-only, so stub the API. */
int ksu_lsm_hook(struct ksu_lsm_hook *hook)
{
	return -ENOSYS;
}
...
void __init ksu_lsm_hook_init(void)
{
	pr_info("lsm_hook: not supported on 4.14, skipped\n");
}
