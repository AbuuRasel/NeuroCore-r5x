...
void track_throne(bool prune_only)
{
	...
	if (!manager_exist) {
		...
		pr_info("Searching manager by name (4.14-safe, no APK scan)...\n");
		{
			struct uid_data *mp;
			static const char * const mgr_pkgs[] = {
				"com.rifsxd.ksunext",
				"com.rifsxd.ksunext.debug",
				NULL,
			};
			int mi;
			for (mi = 0; mgr_pkgs[mi]; mi++) {
				list_for_each_entry (mp, &uid_list, list) {
					if (!strcmp(mp->package, mgr_pkgs[mi])) {
						pr_info("Crowning manager by name: %s (uid=%d)\n",
							mp->package, mp->uid);
						ksu_set_manager_appid(mp->uid);
						manager_exist = true;
						break;
					}
				}
